...
- **SASL against a public network, and the Libera ban on the dogfood IP.** *(not
  blocking)* Stage 1's Done-when names Libera.Chat with SASL; prompt 10b proved the
  whole sequence at OFTC without SASL, and SASL only against local ergo. Two separate
  questions, and neither is a code question. (1) No registered Libera (or OFTC) account
  exists — creating one, and where its password lives, is the user's call. (2) This IP
  is network-banned at Libera as of 2026/8/11 02.58, appealable only by mailing
  `bans@libera.chat`, which is also the user's call and not a thing an automated run
  may do — and worth answering together with the hardcoded `"supernaut debug session"`
  realname the ban message called a bot (stage 3's first carry-forward note). Until both are answered, no run can close the Done-when's SASL clause on a
  public network. Settle before stage 3 (Dogfood month), which is a month of daily use
  on a real network and cannot start banned.
...
10. **Network config and credentials.** TOML config for networks/nick/autojoin as seed
    data; SASL secrets in the OS `keyring`, keyed by the config network's name, with
    the config holding only a `sasl_account` name — never plaintext in the config file
    (§5.8). Ends with the stage acceptance run driven from config alone.
    **The encrypted-file fallback is deferred, deliberately** (prompt 10b): a fallback
    file needs an encryption key, and every honest source of one is out of this stage's
    reach — so what ships is a store-unavailable error that names the deferral rather
    than a file "encrypted" with a key stored beside it. Filed as a carry-forward note
    on stage 4 item 3, whose login-time daemon on a locked Secret Service is its first
    real consumer, and due by stage 6 item 3 where "runs on Linux" becomes a promise.
...
**What prompt 10b's acceptance run actually proved, and the one gap it did not:** every
clause above ran end to end against a real public network — but at **OFTC**, not
Libera.Chat, and **without SASL**. Libera network-banned the dogfood IP mid-prompt
(`465 … Your bot is not permitted to connect`, 2026/8/11 02.58, after a ten-channel
survey run from one client), and no registered Libera account exists to authenticate
with in the first place. SASL from the keyring is proven against local ergo, in
`scripts/live-run.sh` on every run. Both halves of the gap — the ban and the missing
account — are in **Still open**, and the honest measurements are in BUILD-LOG's prompt
10b entry.
...
5. **Input widget and command line.** Composer (client-authoritative while typing,
   ...
   ### Carry-forward
   - From stage 1 prompt 10b: **on a network without `echo-message`, nothing you say
     is ever stored — observed, not theorised.** The stage-1 acceptance run at OFTC
     got exactly one capability back (`CAP * LS :multi-prefix`), so the client sent no
     `CAP REQ` at all, and the line it PRIVMSGed to its own channel is absent from
     `message`, from `backlog` and from `search` — while every inbound line the network
     sent landed. Ingestion is inbound-only by design
     (`crates/havoc-core/src/connection/ingest.rs`), and `echo-message` is what makes
     our own text inbound. A daily-usable client cannot have a half-empty history on a
     pre-IRCv3 network, so this item owes the decision: ingest our own successful sends
     locally when the capability is absent (and reconcile if it is present), or say out
     loud in the UI that this network cannot record what you say. The note below covers
     the *composer-state* half of the same missing capability; this is the *history*
     half, and it is the one a person notices a week later. Stage 3's dogfood month is
     the deadline — a month of use on such a network with no record of your own side is
     not a dogfood, it is a demo.
...
7. **First-run experience.** Pick a network, type a nick, you are on IRC with TLS,
   ...
   - From stage 1 prompt 10b: **there is no no-echo password prompt, and this item
     ...
     when stdin is a TTY, prints one stderr line saying the input is not hidden.
     Suppressing echo means either a new dependency or raw-mode terminal code, and
     the terminal is this item's, where the first-run wizard owns credential entry.
     Stdin stays the seam regardless — it is the one that works for a pipe as well as
     for a person — so this adds a hidden prompt, it does not replace the pipe.
   - From stage 1 prompt 10b: **`sasl_account` beside `plaintext = true` is a config
     error, and that is stricter than §2.3 asks — the same call as the note below.**
     `Config::validate` in `crates/havoc-core/src/config.rs` refuses the pair by name
     because SASL PLAIN puts the password on the wire and plaintext leaves the wire
     readable. The loopback-only rule bounds the exposure to the box, so a user with a
     plaintext LAN bouncer who authenticates to it is the case that argues for
     relaxing this; if the note below relaxes the loopback rule, decide this pair in
     the same breath rather than leaving a plaintext SASL login as an accident.
...
1. **Dogfood fixes.** Use it as the only IRC client for one month; fix what daily use
   ...
   ### Carry-forward
   - From stage 1 prompt 10b: **the USER realname is the const `"supernaut debug
     session"`, and a real network read it as a bot and banned us.** `REALNAME` in
     `crates/havoc-core/src/config.rs` is not configurable and goes out on every
     connection; during prompt 10b's acceptance run Libera network-banned this IP with
     `465 … Your bot is not permitted to connect to Libera Chat`. The realname was not
     provably the cause (a ten-channel survey from one client is the other candidate,
     and both happened), which is exactly why it must not survive a month of daily use
     unexamined: the string literally says "debug". A month cannot start on a network
     that bans you on day one, so before this stage begins the realname becomes real —
     configurable, defaulted to something a human would write — and the ban question in
     **Still open** is answered. Prompt 10a carried the const verbatim on the grounds
     that changing the USER line is an observable protocol change with no benefit;
     reality has now supplied the benefit.
...
3. **Daemon and attach modes.** `supernaut --daemon` / `supernaut --connect <path>`
   ...
   - From stage 1 prompt 10b: **the encrypted-file fallback for SASL secrets is
     ...
     prompt 10b shipped the keyring half only, because a fallback file needs a key and
     stage 1 had no honest source for one (a passphrase needs stage 2's no-echo
     prompt; a KDF + AEAD pair is two dependencies bought for a consumer that did not
     exist). A daemon started at login on a box whose Secret Service is locked — or
     which has no platform store at all — is that consumer: `credentials.rs`'s
     store-unavailable error currently names this deferral by PLAN item, so the gap is
     spoken rather than discovered, and the error text must be updated in the same
     commit that closes it. **Due by stage 6 item 3 (Release)**, where "runs on Linux"
     becomes a user promise. `keyring-core` with an explicit store is the route
     (prompt 10b's decision entry records why plain `keyring` was right for a Mac and
     what would justify revisiting it); the key's provenance is the actual design
     question, not the file format.
